...
import { validateBodyOrder } from '../../utils/validators/order-validator';
import checkAuth from '../../utils/check-auth';
import { Order } from '../../common/exchange-types';
...
const getInstrument: RequestHandler = (req: ExchangeRequest, res: Response, next: NextFunction) => {
...
const getOrderBook: RequestHandler = (req: ExchangeRequest, res, next) => {
    if (orderbookMap.get(req.instrument.name) === undefined) {
        ...
        orderbook.on(OrderBook.TRADE, (buyer: IUser, seller: IUser, price: number, volume: number) => {
            UserDetail.findOneAndUpdate(
                { userId: buyer.id, instrumentId: req.instrument._id },
                { $inc: { position: volume }}
            ...
            UserDetail.findOneAndUpdate(
                { userId: seller.id, instrumentId: req.instrument._id },
                { $inc: { position: -volume }}
